...
describe('QuotaRings', () => {
  ...
  it('keeps total, Build, Imagine, and API visible when product data exists', async () => {
    ...
    expect(details.getByText('每週')).toBeInTheDocument()
  ...
  it('opens quota details on click and keyboard', async () => {
    const user = userEvent.setup()
    render(<QuotaRings billing={billing} now={new Date('2026-07-12T02:38:18Z')} />)
    const reactor = screen.getByTestId('quota-reactor')
    expect(reactor).toHaveAttribute('aria-expanded', 'false')
    await user.click(reactor)
    expect(reactor).toHaveAttribute('aria-expanded', 'true')
    expect(screen.getByRole('region', { name: '週額度明細' })).toBeInTheDocument()
