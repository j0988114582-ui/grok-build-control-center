...
export function App(): React.JSX.Element {
  ...
  const createSession = async (): Promise<void> => { const cwd = await window.grokApi.chooseDirectory(); if (!cwd) return; await connect(); const response = await window.grokApi.createSession(cwd); if (!response.sessionId) return; const summary = { id: response.sessionId, cwd, title: 'New session', updatedAt: new Date().toISOString() }; setSessions((current) => [summary, ...current]); setActive(summary) }
  const loadSession = async (session: SessionSummary): Promise<void> => { setActive(session); setEvents((current) => ({ ...current, [session.id]: [] })); await connect(); try { await window.grokApi.loadSession(session.id, session.cwd) } catch (error) { setNotice(error instanceof Error ? error.message : String(error)) } }
