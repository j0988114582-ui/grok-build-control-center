...
- **子代理官方執行清單（只讀）**：背景活動面板會輪詢 `_x.ai/subagent/list_running`，把 CLI 官方回報的「執行中子代理」合進清單，仍是**一個子代理一張卡**，狀態標成「執行中（官方回報）」並附上類型／輪數／工具次數／token 用量。CLI 答不出來時（舊版或連線異常）自動退回原本的 tee／工具推估，不會變成空白。只在有回合進行中或面板開著時輪詢，閒置視窗不會一直打 CLI。**刻意只讀**：沒有取消子代理的按鈕，也不呼叫 `_x.ai/subagent/cancel`。官方清單只回報「還活著的」子代理，因此卡片消失於清單時**不會**被降級成「已完成」——完成仍以 `subagent_finished` 為準，避免一次不巧的輪詢把還在跑的子代理標成結束。
...
- **專案列＋**：每個資料夾群組標題可直接在該資料夾開新對話，不必再走「選資料夾」。頂部按鈕改為「選資料夾開始」。不做「不在專案底下工作」——Grok ACP 一定要工作目錄。
...
- **規劃核准窗殘留**：Esc、停止回合或回合結束會關掉核准窗；取消／斷線仍不會誤核准開工。
- **權限失敗關窗**：回覆失敗時視窗留著；未連線或未知 request 不再被當成成功。
- **活躍篩選 × 建議清理**：側欄「刪除所選」只算看得見的；建議清理可刪被篩選藏起來的對話，不再靜默沒做事。
- **`/always-approve`**：命令面板、單獨送出、插話、「排隊下一輪」與「立刻改做」都改走既有 YOLO 確認，不再當普通提示送進 CLI。
- **畫面**：右側抽屜改不透明、收合側欄不再漏出資料夾／Team／清理、窄窗保留預覽欄、取消回合改「回合已取消」、使用者標籤改「你」。
- **切換對話**：同一連線裡已打開過的 Session 不再清空重播整段歷史。長對話切走再切回時，畫面沿用快取、立刻還原該則的 Context 用量，也不會堆一排「Grok 正在工作」或跳出「跳到最新 317」。第一次打開仍會走 session/load（CLI 需要），載入結束後把歷史殘留的 running／pending 標成已完成。新建、權限重連、手機焦點載入也會登記快取；斷線會清掉快取。舊的 load 失敗不能回滾較新的成功載入。連線代數對不上時，新建的對話仍會出現在側欄，請再點一次，不會假裝已就緒。載入中仍可點回該則。專案＋若沒拿到 sessionId 會顯示錯誤。Renderer 每個 load 有單調 operation token，成功／失敗／finally 都驗所有權；main 的 `clearIfCurrentLoad` 只讓最新 load 清就緒。remote 焦點載入會搶走最新 token、狀態回音不會誤殺 desktop load、斷線失效所有進行中 load，延遲到達的過期 `loading` 也不會再卡死對話。
